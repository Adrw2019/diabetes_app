...
- **Generar `.env` seguro:** si aún no tienes un `.env` con valores reales, corre `python scripts/setup_env.py`. El script te pide `SECRET_KEY`, `DATABASE_URL`, SMTP y overrides, genera el archivo `.env` y evita que compartas secretos por error.
- **Usuarios de prueba:** si quieres registrar varios usuarios de una sola vez antes de hacer pruebas, ejecuta `python scripts/create_test_users.py`. El script crea tres cuentas (`usuario1`, `usuario2`, `usuario3`) que puedes usar para login/registro/recuperación sin editar más variables.
